...
type 'a context = {
  ...
  mutable annotations : 'a option Int_map.t;
...
let rich_pp annotate ppcmds =
  ...
  let pp_tag obj =
    ...
    let () = context.annotations <- Int_map.add index obj context.annotations in
    string_of_int index
  in
  ...
  let close_xml_tag tag =
    ...
    | Node (node, child, pos, ctx) ->
      ...
      let annotation =
        try Int_map.find (int_of_string node) context.annotations
        with _ -> None
      in
      let annotation = {
