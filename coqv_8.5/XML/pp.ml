...
struct
  ...
  let create (s : string) =
    ...
    let () = assert (not (Int_map.mem hash !dyntab)) in
  ...
  let prj (nh, rv) h =
    if h = nh then Some (Obj.magic rv)
    else None
...
| Notice -> msgnl msg
| Warning -> msgnl_with !err_ft (warnbody msg)
...
| Notice -> msgnl mesg
| Warning -> msgnl_with !err_ft (warnbody mesg)
...
let prvecti_with_sep sep elem v =
  let rec pr i =
    if i = 0 then (*modified by Jian Liu*)
      elem 0 v.(0)
    else
  ...
  if n = 0 then mt () else pr (n - 1) (*modified by Jian Liu*)
