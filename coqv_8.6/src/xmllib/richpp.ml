...
type 'a context = {
  ...
  mutable annotations : 'a option My_map.t; (*modified by Jian Liu*)
...
let rich_pp annotate ppcmds =
  ...
  let context = {
    stack = Leaf;
    offset = 0;
    annotations = My_map.empty; (*modified by Jian Liu*)
    index = (-1);
  } in
  ...
  let pp_tag obj =
    ...
    let () = context.annotations <- My_map.add index obj context.annotations in (*modified by Jian Liu*)
    string_of_int index
  in
  ...
  let close_xml_tag tag =
    ...
    | Node (node, child, pos, ctx) ->
      ...
      let annotation =
        try My_map.find (int_of_string node) context.annotations (*modified by Jian Liu*)
        with _ -> None
      in
      let annotation = {
