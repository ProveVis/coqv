...
let compilation_output_name : string option ref = ref None
...
let async_proofs_private_flags : async_proofs option ref = ref None
let async_proofs_full = ref false
let async_proofs_never_reopen_branch = ref false
let async_proofs_flags_for_workers : (async_proofs list) ref = ref []
...
let async_proofs_tac_error_resilience : tac_error_filter ref = ref (`Only [ "curly" ])
...
let ideslave_coqtop_flags : bool option ref = ref None
